// Private photograph/master stay in output/ and x/. Export one owned paint/load variant.
import sharp from 'sharp';
import { readFile, writeFile } from 'node:fs/promises';
const master = 'output/freight-dhl-custom/master.png';
...
// 85 mm N-scale Lgs 579 chassis × 160 = 13.6 m equivalent; confirm this custom piece by measurement.
const width = 136;
...
const source = '/img/owned/db-lgs579-dhl-weathered.png';
const enhanced = '/img/enhanced/db-lgs579-dhl-weathered-4x.webp';
const zoom = '/img/zoom/db-lgs579-dhl-weathered.webp';
await sharp(native).extend({ top: 0, bottom: height - nativeMeta.height, left: 0, right: 0, background: '#00000000' }).png().toFile(`public${source}`);
...
await writeFile('output/freight-dhl-custom/export.json', JSON.stringify({ source, enhanced, zoom, width, height, crop }, null, 2) + '\n');
